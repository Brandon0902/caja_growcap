{{-- resources/views/movimientos_caja/edit.blade.php --}}
<x-app-layout>
  <x-slot name="header">
  ...
  <div class="py-6 mx-auto max-w-3xl px-4 sm:px-6 lg:px-8"
       x-data="{ tipo: '{{ old('tipo_mov', $movimiento->tipo_mov) }}' }"
  >
      ...
      <form action="{{ route('movimientos-caja.update', $movimiento) }}" method="POST">
          ...
          <select id="id_caja" name="id_caja"
            ...
            @foreach($cajas as $c)
              <option value="{{ $c->id_caja }}"
                {{ old('id_caja', $movimiento->id_caja)==$c->id_caja ? 'selected':'' }}>
            ...
            <select id="proveedor_id" name="proveedor_id"
              ...
              @foreach($proveedores as $p)
                <option value="{{ $p->id_proveedor }}"
                  {{ old('proveedor_id', $movimiento->proveedor_id)==$p->id_proveedor ? 'selected':'' }}>
                  {{ $p->nombre }}
                </option>
              @endforeach
            </select>
        ...
        <div class="mb-4">
          ...
          <x-input id="fecha" name="fecha" type="datetime-local"
                   :value="old('fecha', $movimiento->fecha->format('Y-m-d\\TH:i'))" />
        </div>
