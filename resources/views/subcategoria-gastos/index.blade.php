      ...
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-purple-700">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase">Categoría Padre</th>
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          @forelse($subcategorias as $sub)
            <tr x-show="$el.textContent.toLowerCase().includes(search.toLowerCase())">
              ...
              <td class="px-6 py-4">{{ optional($sub->categoria)->nombre }}</td>
              <td class="px-6 py-4">{{ optional($sub->usuario)->name }}</td>
              ...
              <td class="px-6 py-4 text-right space-x-2">
                ...
                <form action="{{ route('subcategoria-gastos.destroy', $sub) }}"
                      method="POST" class="inline">
                  @csrf @method('DELETE')
                  <button type="button" class="text-red-600 hover:text-red-800 btn-delete"
                          data-id="{{ $sub->id_sub_gasto }}">Eliminar</button>
                </form>
              </td>
            </tr>
          @empty
