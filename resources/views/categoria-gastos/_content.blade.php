...
<div class="py-6 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"
  ...
  <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg overflow-hidden" id="categorias-results">
        ...
        @forelse($categorias as $cat)
          <tr>
            ...
            <td class="px-6 py-4 text-right space-x-2">
              ...
              <form action="{{ route('categoria-gastos.destroy', $cat) }}" method="POST" class="inline">
                @csrf @method('DELETE')
                <button type="button"
                        class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 btn-delete"
                        data-id="{{ $cat->id_cat_gasto }}">
                  {{ __('Eliminar') }}
                </button>
              </form>
            </td>
...
<script>
  function catGastosIndexPage() {
    return {
      ...
      init() {
        this.container = document.getElementById('categorias-results');

        // Paginación AJAX (solo links con page=)
        ...
        // Delegación: borrar con SweetAlert (sigue funcionando tras refrescos)
        this.container?.addEventListener('click', (e) => {
      ...
      buildUrl() {
        const base = @json(route('categoria-gastos.index'));
        const params = new URLSearchParams({
          search: this.search ?? '',
          ajax: '1', // si el backend devuelve fragmento; si no, parseamos
        });
      ...
      async fetchTo(url) {
        ...
        try {
          ...
          // Si viene la página completa, extraemos #categorias-results
          let htmlToInject = text;
          try {
            const doc = new DOMParser().parseFromString(text, 'text/html');
            const frag = doc.querySelector('#categorias-results');
