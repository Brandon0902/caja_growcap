...
<head>
  ...
  <title>Inversión activada</title>
</head>
<body style="font-family:Arial,Helvetica,sans-serif;background:#f6f7fb;margin:0;padding:24px;">
  @php
    ...
    // ✅ NO usar nombre (porque la tabla inversiones no lo tiene)
    $planText = null;
    if ($p) {
      if ($meses) {
    ...
    // ✅ fecha fin: usar guardada; si no, intenta fecha_fin_calc; si no, calcula
    ...
    $metodoTxt = ($metodo === 'saldo') ? 'Saldo disponible' : 'Tarjeta (Stripe)';
  @endphp
  ...
  <div style="max-width:720px;margin:0 auto;background:#ffffff;border-radius:12px;padding:20px;border:1px solid #e7e9f2;">
    ...
    <div style="background:#f9fafb;border:1px solid #eef2f7;border-radius:10px;padding:14px;margin:14px 0;">
      ...
      @if($metodo !== 'saldo' && !empty($stripeMeta['paid_at']))
        <p style="margin:6px 0;color:#111827;"><b>Fecha de pago:</b> {{ $stripeMeta['paid_at'] }}</p>
      @endif
    </div>
      ...
      <div style="background:#ffffff;border:1px solid #eef2f7;border-radius:10px;padding:14px;">
        ...
        @else
          <p style="margin:0;color:#374151;">
            No fue posible detallar el origen del saldo (pero el pago sí se aplicó correctamente).
          </p>
        @endif
      </div>
    @else
      <p style="margin:14px 0 0 0;color:#374151;line-height:1.5;">
        Tu pago con tarjeta fue confirmado correctamente.
      </p>
    @endif
