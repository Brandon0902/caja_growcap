...
<body style="font-family:Arial,Helvetica,sans-serif;background:#f6f7fb;margin:0;padding:24px;">
  @php
    ...
    // Nota: aquí usamos la nota actual como "comentario".
    $comentario = !empty($deposito->nota) ? $deposito->nota : null;
  @endphp

  <div style="max-width:720px;margin:0 auto;background:#ffffff;border-radius:12px;padding:20px;border:1px solid #e7e9f2;">
    <h2 style="margin:0 0 12px 0;color:#111827;">Tu depósito fue rechazado</h2>

    <p style="margin:0 0 14px 0;color:#374151;line-height:1.5;">
      Hola <b>{{ $clienteNombre }}</b>, tu depósito fue revisado por un administrador y no pudo ser aprobado.
    </p>
    ...
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px;margin:14px 0;color:#7f1d1d;">
      Si crees que esto es un error, por favor contacta a soporte o vuelve a generar el depósito con la información correcta.
    </div>
