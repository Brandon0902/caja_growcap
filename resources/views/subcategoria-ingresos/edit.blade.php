{{-- resources/views/subcategoria_ingresos/edit.blade.php --}}
<x-app-layout>
  <x-slot name="header">
  ...
  <div class="py-6 mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
      <form action="{{ route('subcategoria-ingresos.update',$subcategoriaIngreso) }}" method="POST">
        @csrf @method('PUT')
        <div class="mb-4">
          <x-label for="id_cat_ing" value="Categoría" />
          <select id="id_cat_ing" name="id_cat_ing"
            ...
            @foreach($cats as $cat)
              <option value="{{ $cat->id_cat_ing }}"
                {{ old('id_cat_ing',$subcategoriaIngreso->id_cat_ing)==$cat->id_cat_ing?'selected':'' }}>
                {{ $cat->nombre }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-4">
          ...
          <x-input id="nombre" name="nombre" type="text"
                   :value="old('nombre',$subcategoriaIngreso->nombre)" required autofocus />
        </div>
