<x-app-layout>
  ...
  <div class="py-6 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" x-data="{ search: '{{ $search }}' }">
    {{-- Búsqueda --}}
    <div class="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
      <input type="text"
             x-model="search"
             @input.debounce.500ms="window.location = '{{ route('user_inversiones.index') }}?search=' + search"
            ...
            @forelse($clientes as $cliente)
              <tr x-show="$el.textContent.toLowerCase().includes(search.toLowerCase())">
                ...
                <td class="px-6 py-4 text-right">
                  <a href="{{ route('user_inversiones.show', $cliente->id) }}"
