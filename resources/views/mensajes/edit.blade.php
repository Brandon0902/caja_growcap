  ...
  <div class="py-6 mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
      <form action="{{ $action }}"
        ...
        {{-- (mismos campos que en create, pero con old(...,$mensaje->campo)) --}}
        ...
        <div>
          ...
          <select name="id_cliente"
            ...
            <option value="" {{ $mensaje->id_cliente===null? 'selected':'' }}>
              Todos los clientes
            </option>
            @foreach($clientes as $c)
              <option value="{{ $c->id }}"
                {{ old('id_cliente', $mensaje->id_cliente)==$c->id ? 'selected' : '' }}>
                {{ $c->nombre }} {{ $c->apellido }}
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <select name="tipo"
                  class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                         focus:ring-green-500 focus:border-green-500 bg-white dark:bg-gray-700
                         text-gray-900 dark:text-gray-100">
            <option value="1" {{ old('tipo',$mensaje->tipo)=='1'? 'selected':'' }}>Notificación</option>
            <option value="2" {{ old('tipo',$mensaje->tipo)=='2'? 'selected':'' }}>Recordatorio</option>
            <option value="3" {{ old('tipo',$mensaje->tipo)=='3'? 'selected':'' }}>Alerta</option>
          </select>
          @error('tipo')
            <p class="text-red-600 text-sm">{{ $message }}</p>
          @enderror
        ...
        <div>
          ...
          <input type="text"
                 name="asunto"
                 value="{{ old('asunto',$mensaje->asunto) }}"
                 class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                        focus:ring-green-500 focus:border-green-500 bg-white dark:bg-gray-700
                        text-gray-900 dark:text-gray-100"/>
          @error('asunto')
        ...
        <div>
          ...
          <textarea name="cuerpo" rows="4"
                    class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                           focus:ring-green-500 focus:border-green-500 bg-white dark:bg-gray-700
                           text-gray-900 dark:text-gray-100">{{ old('cuerpo',$mensaje->cuerpo) }}</textarea>
          @error('cuerpo')
            <p class="text-red-600 text-sm">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div>
          ...
          <input type="datetime-local"
                 name="fecha_envio"
                 value="{{ old('fecha_envio',$mensaje->fecha_envio?->format('Y-m-d\TH:i')) }}"
                 class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                        focus:ring-green-500 focus:border-green-500 bg-white dark:bg-gray-700
                        text-gray-900 dark:text-gray-100"/>
          @error('fecha_envio')
        ...
        <div>
          ...
          <select name="status"
                  class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                         focus:ring-green-500 focus:border-green-500 bg-white dark:bg-gray-700
                         text-gray-900 dark:text-gray-100">
            <option value="1" {{ old('status',$mensaje->status)=='1'? 'selected':'' }}>Activo</option>
            <option value="0" {{ old('status',$mensaje->status)=='0'? 'selected':'' }}>Inactivo</option>
          </select>
          @error('status')
            <p class="text-red-600 text-sm">{{ $message }}</p>
          @enderror
