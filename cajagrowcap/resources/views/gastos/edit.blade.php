<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-white leading-tight">
      {{ __('Editar transacción entre cajas') }}
    </h2>
  </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
      <form action="{{ route('gastos.update', $gasto) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
          ...
          <x-input id="tipo" name="tipo" type="text" :value="old('tipo', $gasto->tipo)" required autofocus />
        </div>
        ...
        <div class="mb-4">
          ...
          <select id="caja_id" name="caja_id"
            ...
            @foreach($cajas as $c)
              <option value="{{ $c->id_caja }}" {{ old('caja_id', $gasto->caja_id) == $c->id_caja ? 'selected' : '' }}>
        ...
        <div class="mb-4">
          ...
          <select id="destino_caja_id" name="destino_caja_id"
            ...
            @foreach($cajas as $c)
              <option value="{{ $c->id_caja }}" {{ old('destino_caja_id', $gasto->destino_caja_id) == $c->id_caja ? 'selected' : '' }}>
                {{ $c->nombre }} ({{ optional($c->sucursal)->nombre }})
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-4">
          ...
          <x-input id="cantidad" name="cantidad" type="number" step="0.01"
                   :value="old('cantidad', $gasto->cantidad)" required />
        </div>
