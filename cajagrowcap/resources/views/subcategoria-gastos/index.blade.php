{{-- resources/views/subcategoria_gastos/index.blade.php --}}
<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-white leading-tight">
      {{ __('Subcategorías de Gasto') }}
    </h2>
  </x-slot>
  ...
  <div class="py-6 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"
       x-data="subcatGastosIndexPage()">

    <div class="mb-4 flex justify-between items-center gap-3">
      <a href="{{ route('subcategoria-gastos.create') }}"
    ...
    <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg overflow-hidden" id="subcat-results">
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-purple-700 dark:bg-purple-900">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase">{{ __('Categoría Padre') }}</th>
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          @forelse($subcategorias as $sub)
            <tr>
              <td class="px-6 py-4 text-gray-700 dark:text-gray-200">{{ $sub->nombre }}</td>
              <td class="px-6 py-4 text-gray-700 dark:text-gray-200">{{ optional($sub->categoria)->nombre ?? '—' }}</td>
              <td class="px-6 py-4 text-gray-700 dark:text-gray-200">{{ optional($sub->usuario)->name ?? '—' }}</td>
              <td class="px-6 py-4 text-gray-700 dark:text-gray-200">
                {{ optional($sub->created_at)->format('Y-m-d H:i') ?? '—' }}
              </td>
              <td class="px-6 py-4 text-right space-x-2">
                <a href="{{ route('subcategoria-gastos.show', $sub) }}"
                   class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Ver</a>
                <a href="{{ route('subcategoria-gastos.edit', $sub) }}"
                   class="text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300">Editar</a>
                <form action="{{ route('subcategoria-gastos.destroy', $sub) }}" method="POST" class="inline">
                  @csrf @method('DELETE')
                  <button type="button"
                          class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 btn-delete"
                          data-id="{{ $sub->id_sub_gasto }}">
                    Eliminar
                  </button>
                </form>
              </td>
  ...
  {{-- Alpine helpers --}}
  <script>
    function subcatGastosIndexPage() {
      return {
        ...
        init() {
          this.container = document.getElementById('subcat-results');

          // Paginación AJAX (solo enlaces de paginación dentro del contenedor)
          this.container?.addEventListener('click', (e) => {
            const a = e.target.closest('a');
            if (!a || !a.href) return;
            if (!/([?&])page=/.test(a.href)) return; // no interceptar otros links (ver/editar)
            e.preventDefault();
            this.fetchTo(a.href);
          });
        ...
        buildUrl() {
          const base = @json(route('subcategoria-gastos.index'));
        ...
        async fetchTo(url) {
          ...
          try {
            ...
            // Si llega la página completa, extraemos solo #subcat-results
            let htmlToInject = text;
            try {
              const doc = new DOMParser().parseFromString(text, 'text/html');
              const frag = doc.querySelector('#subcat-results');
              if (frag) htmlToInject = frag.innerHTML;
            } catch (_) {}

            if (this.container) {
              this.container.innerHTML = htmlToInject;
              // Re-inicializa Alpine en el nuevo contenido
              if (window.Alpine && Alpine.initTree) Alpine.initTree(this.container);
            }

            // Actualiza la URL (remueve ajax=1)
