    ...
    <div class="py-6 mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
            <form action="{{ route('sucursales.update', $sucursal) }}" method="POST">
                ...
                <div class="mb-4">
                    <x-input-label for="gerente_id" :value="__('Responsable (Gerente/Admin)')" />
                    <select
                        id="gerente_id"
                        name="gerente_id"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700
                    ...
                    >
                        ...
                        @php
                            ...
                            $selected = old('gerente_id', $sucursal->gerente_id);
                        @endphp

                        @if($ger->count())
                            <optgroup label="Gerentes">
                                @foreach($ger as $g)
                                    <option value="{{ $g->id_usuario }}" @selected($selected == $g->id_usuario)>
                                        {{ $g->name }} ({{ $g->email }})
                                    </option>
                                @endforeach
                            </optgroup>
