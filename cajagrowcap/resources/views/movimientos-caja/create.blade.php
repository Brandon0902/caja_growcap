...
<x-app-layout>
  ...
  <div class="py-6 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"
       x-data="{
          tipo: '{{ old('tipo_mov', 'ingreso') }}',
          resetIngreso() {
            const a = document.getElementById('id_cat_ing');
            const b = document.getElementById('id_sub_ing');
            if (a) a.value = '';
            if (b) b.value = '';
          },
          resetGasto() {
            const a = document.getElementById('id_cat_gasto');
            const b = document.getElementById('id_sub_gasto');
            if (a) a.value = '';
            if (b) b.value = '';
          }
       }"
       x-init="
         if (tipo === 'ingreso') resetGasto(); else resetIngreso();
       "
  >
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
      ...
      <form action="{{ route('movimientos-caja.store') }}" method="POST">
        ...
        <div class="grid gap-6">
          ...
          {{-- Bloque de Ingreso --}}
          <div x-cloak x-show="tipo==='ingreso'">
            <div>
            ...
            <div class="mt-4">
              ...
              <select name="id_sub_ing" id="id_sub_ing"
                ...
                @foreach($subsIngreso as $sub)
                  {{-- ✅ FIX: usar id_sub_ing (no id_sub_ingreso) --}}
                  <option value="{{ $sub->id_sub_ing }}"
                    {{ old('id_sub_ing') == $sub->id_sub_ing ? 'selected' : '' }}>
                    {{ $sub->nombre }}
                  </option>
                @endforeach
              </select>
