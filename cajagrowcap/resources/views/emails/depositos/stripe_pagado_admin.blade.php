...
<head>
  ...
  <title>Depósito pagado con Stripe</title>
</head>
<body style="font-family:Arial,Helvetica,sans-serif;background:#f6f7fb;margin:0;padding:24px;">
  ...
  <div style="max-width:720px;margin:0 auto;background:#ffffff;border-radius:12px;padding:20px;border:1px solid #e7e9f2;">
    <h2 style="margin:0 0 12px 0;color:#111827;">Depósito pagado con tarjeta (Stripe)</h2>

    <p style="margin:0 0 14px 0;color:#374151;line-height:1.5;">
      Se confirmó un pago en Stripe. El depósito debe ser revisado y aprobado por un administrador.
    </p>

    <div style="background:#f9fafb;border:1px solid #eef2f7;border-radius:10px;padding:14px;margin:14px 0;">
      <p style="margin:6px 0;color:#111827;"><b>Depósito ID:</b> #{{ $deposito->id }}</p>
      <p style="margin:6px 0;color:#111827;"><b>Cliente:</b> {{ trim($clienteNombre.' '.$clienteApellido) }}</p>
      ...
      @if(!empty($paymentIntentId))
        <p style="margin:6px 0;color:#111827;"><b>Payment Intent:</b> {{ $paymentIntentId }}</p>
      @endif
      @if(!empty($checkoutSessionId))
        <p style="margin:6px 0;color:#111827;"><b>Checkout Session:</b> {{ $checkoutSessionId }}</p>
      @endif
    </div>

    <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:12px;margin:14px 0;color:#7c2d12;">
      Recuerda: este depósito está <b>pagado en Stripe</b> pero sigue <b>pendiente</b> hasta que lo apruebes en el panel.
    </div>
