...
<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      ...
      {{-- ⬇ Volver al show del PRÉSTAMO, no del cliente --}}
      <a href="{{ route('user_prestamos.show', $prestamo) }}"
      ...
      <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6 space-y-6">
        ...
        <div>
          ...
          <select id="id_caja" name="id_caja" required
            ...
            @foreach($cajas as $caja)
              @php
                // fecha_apertura viene como string (o null) desde DB::table
                $fa = $caja->fecha_apertura ?? null;
                $faFmt = $fa ? \Carbon\Carbon::parse($fa)->format('d/m/Y') : 's/fecha';
              @endphp
              <option value="{{ $caja->id_caja }}"
                {{ old('id_caja', $prestamo->id_caja) == $caja->id_caja ? 'selected' : '' }}>
                {{ $caja->nombre }} ({{ $faFmt }})
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <select id="status" name="status" required
            ...
            @foreach($statusOptions as $key => $label)
              <option value="{{ $key }}"
                {{ (string)old('status', $prestamo->status) === (string)$key ? 'selected' : '' }}>
                {{ $label }}
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <textarea id="nota" name="nota" rows="4"
                    class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md
                           bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 focus:ring focus:ring-purple-500">{{ old('nota', $prestamo->nota) }}</textarea>
        </div>
